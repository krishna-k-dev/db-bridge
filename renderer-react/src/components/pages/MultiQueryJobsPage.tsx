...
import { Plus, Search, MoreVertical, Edit, Trash2, Play, Pause, Copy, Sparkle } from 'lucide-react'
...
interface MultiQueryJobsPageProps {
  onCountChange: (count: number) => void
}

const MultiQueryJobsPage = ({ onCountChange }: MultiQueryJobsPageProps) => {
  ...
  const loadJobs = async () => {
    ...
    // Filter only multi-query jobs (jobs with queries array and at least one query)
    const multiJobs = allJobs.filter((job: Job) => job.queries && job.queries.length > 0)
    setJobs(multiJobs)

    const groups = Array.from(new Set(multiJobs.map((j: Job) => j.group).filter(Boolean))) as string[]
    setJobGroups(groups)
  }
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">Jobs with multiple named SQL queries</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-lg shadow">
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-2 py-1.5 text-left text-[10px] font-medium text-gray-500 uppercase tracking-tight">Queries</th>
        ...
        {filteredJobs.length === 0 && (
          <div className="text-center py-12">
            <Sparkle className="w-12 h-12 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-500">No multi-query jobs found</p>
            <button
              onClick={() => navigate('/jobs/create')}
              className="mt-4 text-purple-600 hover:text-purple-700 text-sm"
            >
              Create your first multi-query job
            </button>
          </div>
        )}
      </div>
      ...
      <AlertDialog open={deleteJobId !== null} onOpenChange={() => setDeleteJobId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the multi-query job.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export default MultiQueryJobsPage
